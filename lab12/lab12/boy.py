...
class FreeBoy:
    ...
    def update(self, frame_time):
        ...
        if self.xdir * self.ydir == 0:
            self.x += (self.xdir * distance)
            self.y += (self.ydir * distance)
        else:
            self.x += self.xdir*math.cos(45)*distance
            self.y += self.ydir*math.cos(45)*distance
    ...
    def draw(self):
        ...
        debug_print('x = %d, y = %d, sx = %d, sy = %d ,xdir= %d,ydir =%d' %(self.x, self.y, sx, sy,self.xdir,self.ydir))
